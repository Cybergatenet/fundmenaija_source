<?php
    ...
    if (!empty($Email)) {
        ...
        // } else {()         $Email = mysqli_real_escape_string($conn, $Email);
    ...
    if (!empty($Id_type)) {
        ...
        if ($Id_type !== "International Passport" && $Id_type !== "Driver License" && $Id_type !== "Voters Card" && $Id_type !== "NIN Document") {
            // $Id_type_error = "* Invalid Pincode";
            echo json_encode(
                array("status"=> false,'message' => 'Invalid ID Type', 'id' => $Id_type)
              );
            exit();
        }
        // else{
        //     echo json_encode(
        //         array("status"=> false,'message' => 'another Type chosen', 'id' => $Id_type)
        //       );
        //     exit();
        // }
    ...
    if (!empty($Username)) {
        if (!preg_match_all('/^[A-Za-z]{1}[A-Za-z0-9_$]{5,20}$/', $Username)) {

            // $UsernameError = "* Please Enter Valid Username";
            echo json_encode(
                array("status"=> false,'message' => 'Please Enter Valid Username e.g. Cybergate')
            );
            exit();
        } else {        
            $UsernameError = false;
    ...
    if (!empty($Password)) {
        if (!preg_match_all('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.* )(?=.*[^a-zA-Z0-9]).{8,16}$/m', $Password)) {
            ...
            echo json_encode(
                array("status"=> false,'message' => 'Password must contain Minimum eight characters, at least one uppercase letter, one lowercase letter, one nu()er and one special character')
              );
            exit();
        } else {
        ...
        if ($Adhar_fileSize <= 1000000) {
            ...
            if (in_array($Adhar_file_extention, $Valid_Extention)) {
                ...
                if ($First_Name_error == null && $Last_Name_error == null && $Mobile_Number_error == null && $Email_error == null && $UsernameError == false && $PasswordError == false && $ConfirmPassError == false) {
                    ...
                    $Adhar_Upload = file_put_contents($Adhar_fileTempName, $Adhar_destinationFile);
